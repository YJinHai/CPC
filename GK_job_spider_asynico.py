...
class GkJob:
    ...
    def start(self, url):
        ...
        loop.run_until_complete(asyncio.gather(asyncio.ensure_future(self.start_url(url, response))))

        # self.get_list(url)
        # http://gdit.jobsys.cn/unijob/index.php/wx/jobfair/com_lists?id=70&page_size=10&p=1

    async def start_url(self, url, response):
        ...
        if int(list_row):
            ...
            async with aiohttp.ClientSession(loop=loop) as session:
                for page in range(total_pages):
                    tasks.append(asyncio.ensure_future(self.get_list(url + str(self.page), session)))
                    self.page += 1
    ...
    async def get_info(self, url_id, session):
        ...
        async with session.get(url) as r:
            ...
            for item in data['jobs']:
                self.booth_name.append(data['booth_name'])
            ...
            df.to_excel('GkJob5.xls')

    async def get_list(self, url, session):
        async with session.get(url) as r:
            data = await r.text()
            data = json.loads(data)
            tasks = []
            ...
            async with aiohttp.ClientSession() as s:
                for item in data['items']:
                    tasks.append(asyncio.ensure_future(self.get_info(item['id'], s)))
                return await asyncio.gather(*tasks)
